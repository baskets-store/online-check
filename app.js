...
async function getHiddenProducts() {
    ...
    for (let i = 1; i < pages[0] + 1; i++) {
        await fetch(`https://baskets.webshopapp.com/admin/products/?page=${i}.json`).then(data => data.json()).then(data => products.push(data.products));

    }

await products.forEach(setProducts =>
